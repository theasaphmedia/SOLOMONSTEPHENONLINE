...
const portfolio = [
  {
    ...
    favicon: 'T',
    faviconBg: 'linear-gradient(135deg, #1A3A5C 0%, #0D2340 100%)',
    faviconColor: '#E8C87A',
    bgGradient: 'linear-gradient(135deg, #0D2340 0%, #1A3A5C 40%, #243F66 100%)',
    accent: '#4A90D9',
  },
  {
    num: '02',
    ...
    desc: 'A cross-platform mobile app redefining how ministers prepare and deliver messages. Replaces printed notes, WhatsApp scripture screenshots, and hand signals to projectionists with one seamless, beautiful system.',
    url: '#',
    label: 'Mobile App — Pending Google Play',
    favicon: 'P',
    faviconBg: 'linear-gradient(135deg, #4A148C 0%, #6A1FC2 100%)',
    faviconColor: '#E8C87A',
    bgGradient: 'linear-gradient(135deg, #1A0A2E 0%, #2D1057 40%, #3D1A72 100%)',
    accent: '#AB47BC',
  },
  {
    num: '03',
    ...
    favicon: 'S',
    faviconBg: 'linear-gradient(135deg, #0D1B0D 0%, #1A3A1A 100%)',
    faviconColor: '#C9A84C',
    bgGradient: 'linear-gradient(135deg, #071407 0%, #0D1B0D 40%, #1A2E1A 100%)',
    accent: '#C9A84C',
  },
]
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(280px,1fr))', gap:'clamp(20px,3vw,32px)' }}>
            {portfolio.map((p, i) => (
              <a key={p.num} href={p.url} target={p.url === '#' ? '_self' : '_blank'} rel="noopener noreferrer"
                ...
                style={{ textDecoration:'none', transitionDelay:`${i * 0.1}s`, display:'block', background:'rgba(255,255,255,0.03)' }}
              >
                ...
                <div style={{ position:'relative', overflow:'hidden' }}>
                  ...
                  <div style={{ background:'rgba(20,20,20,0.95)', padding:'10px 14px', display:'flex', alignItems:'center', gap:'10px', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
                    ...
                    <div style={{ flex:1, background:'rgba(255,255,255,0.06)', borderRadius:'4px', padding:'5px 10px', display:'flex', alignItems:'center', gap:'7px', minWidth:0 }}>
                      ...
                      {p.url !== '#' && (
                        <svg style={{ marginLeft:'auto', flexShrink:0 }} width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.25)" strokeWidth="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
                      )}
                    </div>
                  </div>
                  {/* Site preview area */}
                  <div style={{ height:'180px', background:p.bgGradient, position:'relative', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
                    {/* Decorative rings */}
                    ...
                    <div style={{ position:'absolute', width:'140px', height:'140px', borderRadius:'50%', border:`1px solid ${p.accent}25`, top:'50%', left:'50%', transform:'translate(-50%,-50%)' }} />
                    {/* Big favicon letter */}
                    <div style={{ width:'64px', height:'64px', borderRadius:'12px', background:p.faviconBg, border:`1px solid ${p.accent}30`, display:'flex', alignItems:'center', justifyContent:'center', boxShadow:`0 8px 32px ${p.accent}20` }}>
                      <span style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'28px', fontWeight:600, color:p.faviconColor, lineHeight:1 }}>{p.favicon}</span>
                    </div>
                    {/* Label badge */}
                    <div style={{ position:'absolute', bottom:'14px', left:'14px' }}>
                      <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'9px', letterSpacing:'.18em', textTransform:'uppercase', color:`${p.accent}CC`, padding:'4px 10px', border:`1px solid ${p.accent}30`, background:'rgba(0,0,0,0.3)' }}>{p.label}</span>
                    </div>
                  </div>
                </div>
                {/* Card body */}
                <div style={{ padding:'clamp(20px,2.5vw,28px)', background:'rgba(255,255,255,0.02)' }}>
                  <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'10px', letterSpacing:'0.18em', color:'rgba(201,168,76,0.55)', marginBottom:'8px' }}>{p.num} · {p.stack}</div>
                  <h3 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'clamp(20px,2.5vw,26px)', fontWeight:400, color:'#FAF7F2', lineHeight:1.2, marginBottom:'12px' }}>{p.title}</h3>
                  <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'13px', lineHeight:1.75, color:'rgba(250,247,242,0.45)', margin:'0 0 16px' }}>{p.desc}</p>
                  {p.url !== '#' ? (
                    <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'10px', letterSpacing:'.16em', textTransform:'uppercase', color:'#C9A84C', display:'flex', alignItems:'center', gap:'6px' }}>
                      Visit site
                    ...
                    <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'10px', letterSpacing:'.16em', textTransform:'uppercase', color:'rgba(201,168,76,0.4)' }}>Coming Soon</div>
                  )}
                </div>
              </a>
            ))}
