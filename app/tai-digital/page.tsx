...
const portfolio = [
  ...
  { id:'02', name:'PulpitFlow', title:'Real-Time Preaching System', label:'Mobile App', stack:['Flutter','Riverpod','Supabase','API.Bible'], desc:'Redefines how ministers prepare and deliver messages. Replaces printed notes, WhatsApp screenshots, and hand signals to projectionists — one seamless system.', url:'#', img:'/images/solomon-photo2.png', live:false },
...
export default function TaiDigitalPage() {
  ...
  useEffect(() => {
    ...
    /* IntersectionObserver — multiple reveal types */
    const obs = new IntersectionObserver((entries) => {
      entries.forEach(e => {
        if (e.isIntersecting) { e.target.classList.add('is-visible'); obs.unobserve(e.target) }
      })
    }, { threshold: 0.06, rootMargin: '0px 0px -40px 0px' })
    document.querySelectorAll('.rv-wipe,.rv-flip,.rv-twist,.rv-flash,.rv-scale,.rv-count,.rv-left,.rv-right').forEach(el => obs.observe(el))

    /* scramble headline after enter */
    const t2 = setTimeout(() => {
      if (h1Ref.current) scramble(h1Ref.current, 'The Asaph', 1100)
    }, 350)
    const t3 = setTimeout(() => {
      if (h1bRef.current) scramble(h1bRef.current, 'Innovations.', 1000)
    }, 650)
    ...
    const t4 = setTimeout(() => {
      ...
      function typeNext() {
        ...
        if (txtEl) {
          const ch = line[ci]
          txtEl.innerHTML = colorize(line.slice(0, ci + 1))
        }
        ci++
    ...
    document.querySelectorAll('.rv-count').forEach(el => counterObs.observe(el))

    return () => { clearTimeout(t); clearTimeout(t2); clearTimeout(t3); clearTimeout(t4); obs.disconnect(); counterObs.disconnect() }
  }, [])
  ...
  return (
    <main ref={mainRef} style={{ background:'#080808', overflowX:'hidden', color:'#F0EDE8' }}>
      <style>{`
        ...
        .d4{transition-delay:.40s} .d5{transition-delay:.54s}
        ...
        /* ── Gradient border scan on code block ── */
        .code-outer {
          position: relative;
          padding: 1px;
        ...
        .cf { color:#79B8FF; }
        .cs { color:#9ECBFF; }
        .cc { color:rgba(255,255,255,.25); }
        .cv { color:#85E89D; }

        @keyframes blink { 0%,100%{opacity:1} 50%{opacity:0} }

        /* ── Glitch hover on headline ── */
        ...
        .svc-row::before {
          ...
          background: linear-gradient(to bottom, #C9A84C, rgba(201,168,76,.3));
          transform: scaleY(0); transform-origin: top;
          transition: transform .55s cubic-bezier(.16,1,.3,1);
        }
        .svc-row:hover::before { transform: scaleY(1); }

        /* glitch on svc number */
        .svc-num { position:relative; }
        .svc-num::after {
          content: attr(data-n);
          position: absolute; top:0; left:0;
          color: #79B8FF; opacity: 0;
          font-family: inherit;
        }
        .svc-row:hover .svc-num::after {
          animation: numGlitch .4s steps(3) both;
        }
        @keyframes numGlitch {
        ...
        .port-card:hover { border-color: rgba(201,168,76,.3); }
        ...
        .btn-gold {
          ...
          transition:background .3s, letter-spacing .4s;
        }
        .btn-gold:hover { background:#D9B85C; letter-spacing:.22em; }
        ...
        .btn-ghost:hover { border-color:rgba(201,168,76,.5); color:#C9A84C; }
      ...
      <section style={{ minHeight:'100vh', position:'relative', overflow:'hidden', display:'flex', alignItems:'center', background:'linear-gradient(150deg,#060A06 0%,#080808 55%,#060810 100%)', padding:'clamp(120px,14vw,160px) clamp(24px,4.5vw,88px) clamp(72px,9vw,110px)' }}>
        ...
        {/* TAI logo — large background watermark in its real colors */}
        <div style={{ position:'absolute', right:'-4%', top:'50%', transform:'translateY(-50%)', width:'clamp(320px,42vw,540px)', opacity:.055, pointerEvents:'none', zIndex:0, userSelect:'none' }}>
          <Image src="/images/tai-logo.svg" alt="" fill={false} width={540} height={632} style={{ width:'100%', height:'auto' }} />
        </div>

        {/* gold glow */}
        <div style={{ position:'absolute', top:'20%', left:'30%', width:'500px', height:'500px', borderRadius:'50%', background:'radial-gradient(circle,rgba(201,168,76,.055) 0%,transparent 70%)', pointerEvents:'none', zIndex:0 }} />
        ...
        <div style={{ maxWidth:'1320px', margin:'0 auto' }}>
          <div className="stats-grid" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'1px', background:'rgba(255,255,255,.06)' }}>
            {[
              ...
              { val:'∞', raw:'∞', label:'Attention to Detail', sub:'non-negotiable' },
            ].map((s, i) => (
              <div key={i} className="rv-flip" style={{ transitionDelay:(i*.1)+'s', background:'#080808', padding:'clamp(28px,3.5vw,48px) clamp(20px,2.5vw,36px)' }}>
                <div className="rv-count" data-count={s.raw} style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'clamp(40px,5vw,64px)', fontWeight:400, color:'#C9A84C', lineHeight:1, marginBottom:'10px' }}>{s.val}</div>
        ...
        <div style={{ maxWidth:'1320px', margin:'0 auto' }}>
          <div className="ab-grid" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'clamp(56px,7vw,104px)', alignItems:'start' }}>
            <div>
              ...
              <h2 className="rv-twist d1" style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'clamp(30px,4.5vw,58px)', fontWeight:400, color:'#F0EDE8', lineHeight:1.1, margin:'0 0 clamp(20px,2.5vw,32px)', letterSpacing:'-.01em' }}>
      ...
      <section style={{ background:'#080808', padding:'clamp(80px,10vw,128px) clamp(24px,4.5vw,88px)' }}>
        <div style={{ maxWidth:'1320px', margin:'0 auto' }}>
          ...
          {services.map((s, i) => (
            <div key={s.id} className="svc-row rv-flip" style={{ transitionDelay:(i*.08)+'s' }}
              onMouseEnter={() => setHovSvc(i)} onMouseLeave={() => setHovSvc(null)}>
              <div className="svc-num" data-n={s.id} style={{ fontFamily:"'Space Mono',monospace", fontSize:'11px', color:'rgba(201,168,76,.4)', paddingTop:'4px', position:'relative' }}>{s.id}</div>
              <div>
                <div style={{ fontFamily:"'Space Mono',monospace", fontSize:'9px', letterSpacing:'.14em', color:'rgba(201,168,76,.32)', marginBottom:'8px' }}>{s.fn}</div>
                ...
                <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'clamp(13px,1.2vw,15px)', lineHeight:1.82, color:'rgba(240,237,232,.32)', margin:0 }}>{s.desc}</p>
              </div>
              <div className="svc-tech" style={{ display:'flex', flexDirection:'column', gap:'6px', paddingTop:'4px', minWidth:'130px' }}>
                {s.tech.map(t => (
                  <span key={t} style={{ fontFamily:"'Space Mono',monospace", fontSize:'9px', letterSpacing:'.12em', color:'rgba(255,255,255,.2)', whiteSpace:'nowrap' }}>{'→'} {t}</span>
                ))}
              </div>
            </div>
          ))}
          ...
          <div className="port-grid" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'clamp(14px,2vw,20px)' }}>
            {portfolio.map((p, i) => (
              <a key={p.id} href={p.url !== '#' ? p.url : undefined} target={p.url !== '#' ? '_blank' : undefined} rel="noopener noreferrer"
                className="port-card rv-twist" style={{ transitionDelay:(i*.12)+'s' }}>
                ...
                <div style={{ padding:'clamp(18px,2.5vw,28px)', flex:1, display:'flex', flexDirection:'column' }}>
                  ...
                  {p.url !== '#' && (
                    <div style={{ marginTop:'18px', fontFamily:"'Space Mono',monospace", fontSize:'10px', letterSpacing:'.16em', color:'rgba(201,168,76,.5)' }}>
                      {'→ visit ' + p.name}
                    </div>
                  )}
                </div>
      ...
      <section style={{ background:'#080808', padding:'clamp(96px,12vw,152px) clamp(24px,4.5vw,88px)', borderTop:'1px solid rgba(255,255,255,.04)', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:'820px', margin:'0 auto', textAlign:'center', position:'relative', zIndex:1 }}>
          ...
          <h2 className="rv-twist d1" style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'clamp(40px,6.5vw,80px)', fontWeight:400, color:'#F0EDE8', lineHeight:1.05, margin:'0 0 clamp(20px,2.5vw,32px)', letterSpacing:'-.01em' }}>
...
/* util: colorize a partial code line */
